...
test_that("Silent", {
  obj <- readRDS(test_path("data", "annotation_results.rds"))
  expect_silent(comp <<- av_complexity(obj$mp))
  expect_silent(hard <<- av_hardlimit_artifact(obj$mp))
  expect_silent(motion <<- av_motion_artifact(obj$mp))
  expect_silent(stopw <<- av_stop_word(obj$mp, stop_word_loc = 150))
  expect_silent(zero <<- av_zerocrossing(obj$mp))
})

test_that("Apply", {
  obj <- readRDS(test_path("data", "annotation_results.rds"))
  comp <- av_complexity(obj$mp)
  avcomp <- av_complexity(obj$mp, apply = TRUE)
