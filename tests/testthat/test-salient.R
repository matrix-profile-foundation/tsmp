...
if (!is.null(res)) {
  test_that("Score", {
    ...
    expect_equal(get_bitsize(data > 0, 10), 5490)
    expect_equal(sum(discrete_norm(data, 3, max(data), min(data))), 546)
    maxmin <- discrete_norm_pre(as.vector(data), 100)
    ...
    expect_equal(sum(get_sorted_idx(res$mp, 10) %in% c(36, 408, 37, 407, 35, 200, 9, 199, 10, 406)), 10)
