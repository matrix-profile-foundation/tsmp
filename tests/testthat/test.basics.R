...
library(tsmp)
w <- 30
ref_data <- toy_data$data[, 1]
query_data <- toy_data$data[, 1]
d_size <- length(ref_data)
q_size <- length(query_data)

test_that("Errors", {
  # big window size
  expect_error(fast_movsd(toy_data$data[, 1], 1), regexp = "must be at least 2")
  expect_error(fast_movsd(toy_data$data[1:100, 1], 500), regexp = "is too large")
...
movsd <- fast_movsd(toy_data$data[, 1], 30)
movavg <- fast_movavg(toy_data$data[, 1], 30)
...
fs1 <- sdts_f_score(test_data$test$label, pred, 1)
fs2 <- sdts_f_score(test_data$train$label, pred, 1)
