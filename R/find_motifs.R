...
find_motifs <- function(.mp, data, n_motifs = 3, radius = 3, mode = c("guided", "unguided"), n_bit = 4, exclusion_zone = NULL, n_dim = NULL) {
  if (!any(class(.mp) %in% c("MatrixProfile", "MultiMatrixProfile"))) {
    stop("Error: First argument must be an object of class `MatrixProfile` or `MultiMatrixProfile`.")
  }
  ...
  switch(class(.mp)[1],
    ...
    "MatrixProfile" = {
      ...
      .mp$motif <- list(motif_idx = motif_idx[[1]], motif_dim = rep(1, n_motifs), motif_neighbor = motif_idx[[2]])
      class(.mp) <- append(class(.mp), "Motif")
      return(.mp)
    },
    # Multidimension ------------------------------------------------------------------------
    "MultiMatrixProfile" = {
      ...
      if (algo == "guided") {
        ...
        class(.mp) <- append(class(.mp), "MultiMotif")
        return(.mp)
      } else {
        # Unguided Search -------------------------------------------------------------------
        if (n_bit < 2) {
