...
#' @export
#'
#' @seealso [mstomp()], [mstomp_par()], [m_motif_usearch()]
...
#' motifs <- m_motif_search(mp_toy_data$data[1:200,], w, mp$mp, mp$pi, 2)
#' \dontrun{
#' w <- mp_toy_data$sub_len
#' mp <- mstomp_par(mp_toy_data$data, w, verbose = 0)
#' motifs <- m_motif_search(mp_toy_data$data, w, mp$mp, mp$pi, 2)
#' }

m_motif_search <- function(data, window_size, matrix_profile, profile_index, n_dim) {
