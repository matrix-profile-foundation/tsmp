...
`[.PMP` <- function(x, ..., drop = FALSE) {
  cli_abort("Subsetting {.cls PMP} is not implemented yet")
}
...
`[.MatrixProfile` <- function(x, ..., drop = FALSE) {
  ...
  if (is.numeric(subset)) {
    ...
    if (!all(diff(subset) == 1)) {
      cli_abort("Indexes must be continuous and ascending.")
    }

    if (sub_size < (2 * max(x$w))) {
      cli_abort("Subset must be larger than twice the window size: 2 * {max(x$w)}")
    }
    ...
    if (max(subset) > max_valid_idx) {
      cli_abort("Index is larger than data size.")
...
`[.MPdistProfile` <- function(x, ..., drop = FALSE) {
  ...
  if (is.numeric(subset)) {
    ...
    if (sub_size < attr$query_size) {
      cli_abort("Subset must be at least the size of query data {attr$query_size}")
    }
...
`[.Snippet` <- function(x, ..., drop = FALSE) {
  ...
  cli_inform("Not implemented yet.")
