...
#' @param k an `int`. Number of discords to find. (Default is `3`).
...
discords <- function(profile, exclusion_zone = profile$ez, k = 3L, neighbor_count = 10L, radius = 3) {
  find_discord(profile, n_discords = k, n_neighbors = neighbor_count, radius = radius, exclusion_zone = exclusion_zone)
}
