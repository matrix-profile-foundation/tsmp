...
pmp <- function(data,
  ...
  if (!is.null(pmp_obj)) {
    if (!inherits(pmp_obj, "PMP")) {
      cli_abort("{.arg pmp_obj} must be of class {.cls PMP}")
    }
  }
  ...
  for (i in seq_along(split_idx)) {
    ...
    if (is.na(w) || w == 0) {
      cli_warn("Invalid window size ", w)
      next
    }
...
pmp_upper_bound <- function(data,
  ...
  while (window_size <= max_window) {
    ...
    if (is.null(result) || result$partial) {
      cli_warn("The computation was terminated prematurely. The results are partial.")
      return()
    }
    ...
    if (correlation_max < threshold) {
      # cli_inform("break at ", window_size)
      break
    }
  ...
  if (window_size <= max_window) {
    ...
    for (window_size in test_windows) {
      # cli_inform("refine window: ", window_size)
      ...
      if (correlation_max < threshold) {
        # cli_inform("break refine at ", window_size)
        break
      }
    }
  }
